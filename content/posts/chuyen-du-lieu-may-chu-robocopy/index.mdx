...
```sql
-- Restore ANBINH
RESTORE DATABASE [ANBINH] 
FROM DISK = N'\\IP_SERVER_CU\Backup\ANBINH.bak' 
WITH FILE = 1, 
MOVE N'ANBINH' TO N'D:\SQLData\ANBINH.mdf', 
MOVE N'ANBINH_log' TO N'D:\SQLData\ANBINH_log.ldf', 
NOUNLOAD, REPLACE, STATS = 5;
GO

-- Restore CHICONG
RESTORE DATABASE [CHICONG] 
FROM DISK = N'\\IP_SERVER_CU\Backup\CHICONG.bak' 
WITH FILE = 1, 
MOVE N'CHICONG' TO N'D:\SQLData\CHICONG.mdf', 
MOVE N'CHICONG_log' TO N'D:\SQLData\CHICONG_log.ldf', 
NOUNLOAD, REPLACE, STATS = 5;
GO
...
1. **Sử dụng Notepad++ / VS Code:**
   ...
   * Sử dụng tính năng **Find & Replace (Ctrl + H)**: Thay thế nhanh tên database `ANBINH` bằng tên các database thực tế khác của bạn.
...
* **Kiểm tra chính xác Logical File Name:** Trước khi chạy script Restore, hãy dùng lệnh `RESTORE FILELISTONLY` để kiểm tra chính xác tên file logic (tên trong mệnh đề `MOVE N'...'`) của từng database. Đôi khi tên database hiển thị là `ANBINH` nhưng tên file logic thực tế bên trong có thể là `ANBINH_Data`, `ANBINH_Primary` hoặc một tên khác tùy theo cách cấu hình ban đầu.
  ```sql
  RESTORE FILELISTONLY FROM DISK = N'D:\Backup_Databases\ANBINH.bak'
  ```
