...
* **Kiểm tra chính xác Logical File Name (Tránh lỗi Msg 3234):** Trước khi chạy script Restore, hãy dùng lệnh `RESTORE FILELISTONLY` để kiểm tra chính xác tên file logic (tên trong mệnh đề `MOVE N'...'`) của từng database.
  ...
  ```
  Điều này nghĩa là tên logic lưu trong file backup khác với tên bạn chỉ định trong lệnh `MOVE`. Ví dụ, nhiều database trong hệ thống multi-tenant được clone từ một DB gốc tên là `GOC` thì tên logic của chúng vẫn sẽ là `GOC` và `GOC_log`.
