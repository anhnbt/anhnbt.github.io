...
FROM DISK = N'\\IP_SERVER_CU\Backup\DB_Demo1.bak' 
WITH FILE = 1, 
MOVE N'DB_Demo1' TO N'D:\SQLData\DB_Demo1.mdf', 
MOVE N'DB_Demo1_log' TO N'D:\SQLData\DB_Demo1_log.ldf', 
NOUNLOAD, REPLACE, STATS = 5;
GO

-- Restore DB_Demo2
RESTORE DATABASE [DB_Demo2] 
FROM DISK = N'\\IP_SERVER_CU\Backup\DB_Demo2.bak' 
WITH FILE = 1, 
MOVE N'DB_Demo2' TO N'D:\SQLData\DB_Demo2.mdf', 
MOVE N'DB_Demo2_log' TO N'D:\SQLData\DB_Demo2_log.ldf', 
NOUNLOAD, REPLACE, STATS = 5;
GO
...
2. **Sử dụng SQL Query để sinh code tự động:** Bạn cũng có thể chạy lệnh SQL sau trên Server cũ để tự động sinh ra chuỗi các câu lệnh Restore một cách chính xác nhất:
   ...
   ```
3. **Sử dụng PowerShell để tự động quét thư mục và xuất file script:**
   Để xuất trực tiếp ra file `.txt` hoặc `.sql` thay vì hiện trên màn hình (tránh việc copy từ màn hình console bị xuống dòng sai lệch), bạn chỉ cần chạy đoạn script sau trong cửa sổ PowerShell:

   ```powershell
   # Đường dẫn chứa file bak
   $backupPath = "C:\Backup_Databases"
   # Đường dẫn dữ liệu SQL Server mới
   ...
   foreach ($file in $backups) {
       ...
       $ldfPath = "$sqlDataPath\$dbName_log.ldf"
       
       # Tạo câu lệnh và lưu vào mảng
       $cmd = "RESTORE DATABASE [$dbName] FROM DISK = N'$($file.FullName)' WITH FILE = 1, MOVE N'$dbName' TO N'$mdfPath', MOVE N'$dbName" + "_log' TO N'$ldfPath', NOUNLOAD, REPLACE, STATS = 5; GO"
       $result += $cmd
   }
